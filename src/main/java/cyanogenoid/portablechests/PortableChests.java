...
public final class PortableChests extends JavaPlugin {
    ...
    public void onDisable() {
        if (database != null) {
            if (database.getTaskId() != -1) {
                Bukkit.getScheduler().cancelTask(database.getTaskId());
                database.save();
            }
        }
    }
    ...
    public static ItemStack makePortableContainer(Inventory inventory, ItemStack itemStack) {
        ...
        PortableChests.instance.database.saveContent(uuid , encodedInventory);
    ...
    private static String getItemStackContentData(ItemStack item) {
        ...
        return PortableChests.instance.database.loadContent(item.getItemMeta().getPersistentDataContainer().get(UNIQUE_KEY, PersistentDataType.STRING));
    }
